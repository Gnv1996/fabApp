...
function PlastUpdate({navigation}) {
  ...
  const updateExpoHandler = async () => {
    try {
      ...
      if (responseData.status) {
        ...
        Alert.alert('Success', 'Plast Expo data updated successfully', [
          {
            text: 'OK',
  ...
  const fetchDataFromAPI = async () => {
    try {
      ...
      const filteredExhibition = allExhibitions.find(
        exhibition => exhibition.title === 'PlastIndia',
      );
